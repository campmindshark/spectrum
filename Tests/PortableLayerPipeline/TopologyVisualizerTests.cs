...
namespace Spectrum.LayerPipeline.Tests {
  ...
  public sealed class TopologyVisualizerTests {
    ...
    public void ArcLightningUsesPhysicalGraph() {
      ...
      Assert.IsTrue(physicalGraph.VertexCount == 71 &&
      ...
      foreach (ImmutableArray<int> branch in path.BranchStruts) {
        ...
        foreach (int strut in branch) {
          Assert.IsTrue(usedStruts.Add(strut),
            "Arc Lightning branch reused a main or branch strut");
        }
      }
      Assert.IsTrue(path.BranchStruts.Length == 2,
      ...
      Assert.IsTrue(poleFan.SequenceEqual(new ArcLightningPolePair[] {
      ...
      Assert.IsTrue(cappedPoleFan.Length == 8 &&
      ...
      Assert.IsTrue(LEDDomeArcLightningVisualizer.BuildPoleFan(
      ...
      Assert.IsTrue(oneWandFallback == new ArcLightningPolePair(
      ...
      Assert.IsTrue(noWandFallback.Origin >= 0 &&
      ...
      Assert.IsTrue(LEDDomeArcLightningVisualizer.VisibleEdgeCount(10, 1, 1) == 10,
      ...
      Assert.IsTrue(lightning != null, "Arc Lightning renderer was not created");
    ...
    public void GlassMosaicUsesTriangleTopology() {
      ...
      Assert.IsTrue(clamped.TileGrouping == 6 && clamped.CascadeSpeed == 1 &&
      ...
      Assert.IsTrue(waveOrder.Length == topology.TileCount &&
      ...
      for (int index = 1; index < waveOrder.Length; index++) {
        ...
        Assert.IsTrue(topology.NeighborsAt(tile).Any(reached.Contains),
          "Glass Mosaic cascade jumped across disconnected faces");
        reached.Add(tile);
      }
      int secondTile = waveOrder[1];
      int secondInitialPhase = secondTile & 7;
      Assert.IsTrue(state.PhaseAt(0) == 1 && state.PreviousPhaseAt(0) == 0 &&
      ...
      Assert.IsTrue(state.PhaseAt(secondTile) == secondInitialPhase,
        "Glass Mosaic cascade ignored its tile-rate budget");
      state.AdvanceCascade(0.01);
      Assert.IsTrue(state.PhaseAt(secondTile) == ((secondInitialPhase + 1) & 7),
      ...
      Assert.IsTrue(mosaic != null, "Glass Mosaic renderer was not created");
      ...
      Assert.IsTrue(mosaic.LayerBuffer.pixels.Any(pixel => pixel.color != 0),
    ...
    public void CellularDomeUsesTriangleCells() {
      ...
      Assert.IsTrue(clamped.Rule == 3 && clamped.Neighborhood == 1 &&
      ...
      for (int tile = 0; tile < topology.TileCount; tile++) {
        ...
        Assert.IsTrue(edgeNeighbors.All(vertexNeighbors.Contains),
        ...
        foreach (int neighbor in vertexNeighbors) {
          Assert.IsTrue(topology.VertexNeighborsAt(neighbor).Contains(tile),
      ...
      Assert.IsTrue(CellularDomeState.NextAlive(1, false, 1) &&
          !CellularDomeState.NextAlive(1, true, 1),
        "Cellular Dome oscillator rule did not implement B1/S0");
      Assert.IsTrue(CellularDomeState.NextAlive(2, true, 0) &&
      ...
      Assert.IsTrue(state.ColorPhaseAt(0, 3, 2) == 4,
        "Cellular Dome age did not advance from the birth color");
      state.MutateAt(0, 0, 0);
      Assert.IsTrue(!state.AliveAt(0) && state.BrightnessAt(0) > 0,
      ...
      Assert.IsTrue(cellular != null, "Cellular Dome renderer was not created");
      ...
      Assert.IsTrue(cellular.LayerBuffer.pixels.Any(pixel => pixel.color != 0),
