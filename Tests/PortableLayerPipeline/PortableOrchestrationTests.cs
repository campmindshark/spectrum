...
namespace Spectrum.LayerPipeline.Tests {
  ...
  [TestClass]
  [DoNotParallelize]
  public sealed class PortableOrchestrationTests {
    [TestMethod]
    public void BuiltInFeaturesLiveInPortableCore() {
      ...
      Assert.IsTrue(catalogType.GetProperty("Default") == null,
        "Base still owns the application feature catalog");
      Assert.IsTrue(typeof(RadialLayerOptions).Assembly == coreType.Assembly,
        "typed built-in options are outside the portable core");
      Assert.IsTrue(coreType.Assembly == runtimeType.Assembly,
      ...
      Assert.IsTrue(catalogType.Assembly.GetType(radialOptionsName) == null,
        "Base still contains built-in renderer option types");
      Assert.IsTrue(DomeLayerCatalog.Metadata.Definitions.Count > 0 &&
    ...
    [TestMethod]
    public void AstronomyOptionsAndHeading() {
      ...
      Assert.IsTrue(options.NorthHeading == 359 &&
      ...
      Assert.IsTrue(defaultOptions.ShowDaytimeSky &&
      ...
      Assert.IsTrue(DomeLayerDate.TryDecode(defaultOptions.StartDate, out _),
        "astronomy start date did not default to a valid local date");
      Assert.IsTrue(DomeLayerDate.TryParse("2026-07-15", out double encodedDate) &&
      ...
      Assert.IsTrue(baseTime == new DateTime(
      ...
      Assert.IsTrue(winterMidnight == new DateTime(
      ...
      Assert.IsTrue(Math.Abs(endJulianDay - baseJulianDay - 7) < 1e-9,
      ...
      Assert.IsTrue(completed && stopped == 168,
      ...
      Assert.IsTrue(!completed && wrapped == 1,
      ...
      Assert.IsTrue(playbackVisualizer != null,
      ...
      playbackVisualizer.Visualize();
      Assert.IsTrue(playbackVisualizer.PlaybackActive,
      ...
      Assert.IsTrue(stoppedOffset >= 10,
      ...
      Assert.IsTrue(Vector3.Distance(northAtZero, Vector3.UnitY) < 1e-6f,
        "zero heading did not put north on projected +Y");
      Assert.IsTrue(Vector3.Distance(eastAtZero, Vector3.UnitX) < 1e-6f,
        "zero heading did not put east on projected +X");
      Assert.IsTrue(Vector3.Distance(northAtNinety, Vector3.UnitX) < 1e-6f,
    ...
    [TestMethod]
    public void ControlStormAvoidsPlanWork() {
      ...
      DomeShowStateSnapshot afterGlobal =
        ((IDomeShowStateConfiguration)config).DomeShowStateSnapshot;
      Assert.IsTrue(beforeGlobal.Palettes.Equals(afterGlobal.Palettes),
    ...
    [TestMethod]
    public void MidiBindingsPublishStateCommands() {
      var config = new global::Spectrum.SpectrumConfiguration();
      ...
      Assert.IsTrue(binding.callback != null,
      ...
      Assert.IsTrue(Math.Abs(config.domeBrightness - 0.6) < 0.000001,
