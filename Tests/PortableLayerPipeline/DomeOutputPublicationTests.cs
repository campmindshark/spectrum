...
namespace Spectrum.LayerPipeline.Tests {
  ...
  public sealed class DomeOutputPublicationTests {
    ...
    public void NativeFrameMailbox() {
      ...
      publisher.FlushFrame(simulationEnabled: true);
      Assert.IsTrue(publisher.NativeCommands.TryDequeue(out var flush) &&
          flush.isFlush,
      ...
      Assert.IsTrue(!publisher.TryTakeNativeFrame(out _),
    ...
    [TestMethod]
    public void IndependentWebChannel() {
      var publisher = new DomeSimulatorPublisher {
        WebHasConsumer = true,
      };
      ...
      Assert.IsTrue(publisher.WebCommands.IsEmpty &&
          hadFrame && frame != null &&
          frame[0] == 0x010203 &&
          frame[1] == 0xA0B0C0,
      ...
      Assert.IsTrue(!publisher.TryTakeWebFrame(out _) &&
          publisher.WebCommands.IsEmpty,
        "closing the browser consumer retained publication state");
    }

    [TestMethod]
    public void RenderStateCapturesOneGeneration() {
      ...
      Assert.IsTrue(ReferenceEquals(captured, firstShow) &&
    ...
    [TestMethod]
    public void OutputSettingsReconcileAtOwnerBoundary() {
      var config = new global::Spectrum.SpectrumConfiguration();
      ...
      Assert.IsTrue(coordinator.AppliedMappingGeneration != mappingGeneration &&
          ReferenceEquals(mapper.Current, initialMapping),
      ...
      coordinator.EnsureApplied();
      Assert.IsTrue(coordinator.AppliedTransportGeneration == transportGeneration &&
          notifications == 2,
        "transport reconciliation did not isolate a settings observer");
    }

    [TestMethod]
    public void RenderPipelineOwnsVisualizerSnapshots() {
      ...
      Assert.IsTrue(initial.Length == 1 &&
      ...
      Assert.IsTrue(!ReferenceEquals(initial, expanded) &&
          expanded.Length == 2 &&
      ...
      Assert.IsTrue(!ReferenceEquals(expanded, reduced) &&
          reduced.Length == 1 &&
    ...
    [TestMethod]
    public void RenderPipelinePublishesBeforeHueMutation() {
      DomeTopology topology = LayerPipelineTestFixtures.OnePixelTopology();
      ...
      Assert.IsTrue(publishedSourceColor == 0xFF0000,
        "persistent layer hue changed before frame publication");
      Assert.IsTrue(renderer.Frame.pixels[0].color != 0xFF0000,
