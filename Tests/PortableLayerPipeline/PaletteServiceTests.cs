...
namespace Spectrum.LayerPipeline.Tests {
  ...
  [TestClass]
  [DoNotParallelize]
  public sealed class PaletteServiceTests {


    [TestMethod]
    public void PaletteMutations() {
      ...
      Assert.IsTrue(added, addError);
      Assert.IsTrue(config.domePalettes.Length == 4 &&
          config.domePalettes[3].Name == "Warm variation",
        "palette copy was not appended");
      Assert.IsTrue(config.domePalettes[0].Colors[0] ==
      ...
      Assert.IsTrue(renamed, renameError);
      Assert.IsTrue(config.domePalettes[1].Name == "Ocean",
        "palette was not renamed in place");

      (bool deleted, string? deleteError) = service.Delete("Ocean");
      Assert.IsTrue(deleted, deleteError);
      Assert.IsTrue(config.domePalettes.Length == 3 &&
          config.domePalettes[1].Name == "Mono",
        "deleted palette remained in the ordered list");
      Assert.IsTrue(config.domeLayerStack[0].RendererParams["palette"] == 0 &&
    ...
    public void ScenePaletteOwnership() {
      ...
      Assert.IsTrue(replaced, replaceError);
      (bool applied, string? applyError) = scenes.Apply("Look");
      Assert.IsTrue(applied, applyError);
      Assert.IsTrue(config.domePalettes[0].GetSingleColor(0) == 0x13579B,
    ...
    private static void AssertColor(
      ...
      Assert.IsTrue(actual != null, message + ": color is null");
      Assert.IsTrue(actual.Color1 == color1, message + ": first endpoint changed");
      Assert.IsTrue(actual.IsGradient == color2.HasValue,
        message + ": gradient state changed");
      if (color2.HasValue) {
        Assert.IsTrue(actual.Color2 == color2.Value,
